...
This document was auto-generated from `actions/iOS-26/compiled.json` (source: `actions\iOS-26\compiled.json`).
...
## Add PDF to Books (`com.apple.iBooks.openin`)
...
**Output Types:** `com.apple.shortcuts.WFContentItem`
...
**Output Types:** `com.apple.shortcuts.WFTrelloCardContentItem`
...
**Output Types:** `com.apple.shortcuts.WFEvernoteContentItem`
...
Prompt the user with a list of menu items and run the matching branch.

> Each menu block uses the same `GroupingIdentifier`. Set `WFControlFlowMode` to 0 for the opening "Choose from Menu" action, 1 for each menu branch, and 2 for the final End Menu block.
> Provide the list of choices via `WFMenuItems`, and optionally set `WFMenuPrompt` to customize the alert text shown to the user.

| Key | Display Name | Description |
| --- | --- | --- |
| `WFMenuItems` | Menu Items |  |
| `WFMenuPrompt` | Prompt |  |
| `WFMenuItemTitle` | Menu Item Title |  |
| `GroupingIdentifier` | Grouping Identifier |  |
| `WFControlFlowMode` | Control Flow Mode |  |
| `UUID` | UUID |  |
...
**Output Types:** `measurement_length`
...
## Create QR Code (`is.workflow.actions.generatebarcode`)
...
| --- | --- | --- |
| `WFText` | Text |  |
| `WFQRForegroundColor` | Foreground Color |  |
| `WFQRBackgroundColor` | Background Color |  |
| `WFQRRounded` | Rounded |  |
...
**Output Types:** `com.apple.shortcuts.WFTrelloBoardContentItem`
...
**Output Types:** `com.apple.shortcuts.WFTrelloListContentItem`
...
| Key | Display Name | Description |
| --- | --- | --- |
| `WFDateActionMode` | Date |  |
| `WFDateActionDate` | Date |  |
| `WFEventOccurrenceMode` | Occurrence |  |
| `WFEventOccurrenceSpecifiedYear` | Year |  |
...
**Output Types:** `com.apple.shortcuts.WFCalendarEventContentItem`
...
**Output Types:** `com.apple.shortcuts.WFReminderContentItem`
...
**Output Types:** `com.apple.shortcuts.WFEmailAddressContentItem`
...
**Output Types:** `com.apple.shortcuts.WFArticleContentItem`
...
**Output Types:** `com.apple.shortcuts.WFEKParticipantContentItem`
...
**Output Types:** `com.apple.shortcuts.WFDisplayContentItem`
...
Searches for the health samples in your library that match the given criteria.

**Output Types:** `com.apple.shortcuts.WFHKSampleContentItem`

| Key | Display Name | Description |
| --- | --- | --- |
| `WFContentItemFilter` | WFContentItemFilter |  |
| `WFHKSampleFilteringUnit` | Unit | The unit to use for the returned results |
| `WFHKSampleFilteringGroupBy` | Group by | The time period to group results by. For example, grouping by day gives you only the daily totals. |
| `WFHKSampleFilteringFillMissing` | Fill Missing | Whether or not to fill in empty groups with zero values. This is useful to turn off when averaging over a longer period of time. |
| `WFContentItemSortProperty` | Sort by | Optionally, what to sort the health samples by. |
| `WFContentItemSortOrder` | Order | The order to sort the health samples in. |
| `WFContentItemLimitEnabled` | Limit | Whether or not to limit the number of health samples retrieved. |
| `WFContentItemLimitNumber` | Get | The maximum number of health samples. |
| `WFCompoundType` | WFCompoundType |  |
| `WFContentItemInputParameter` | health samples |  |
...
**Output Types:** `com.apple.shortcuts.WFiTunesProductContentItem`
...
**Output Types:** `com.apple.shortcuts.WFMKMapItemContentItem`
...
**Output Types:** `com.apple.shortcuts.WFPodcastShowContentItem`
...
**Output Types:** `com.apple.shortcuts.WFWindowContentItem`
...
**Output Types:** `com.apple.shortcuts.WFPosterContentItem`
...
**Output Types:** `com.apple.shortcuts.WFFocusModeContentItem`
...
**Output Types:** `com.apple.shortcuts.WFWeatherDataContentItem`
...
**Output Types:** `com.apple.shortcuts.WFFileSizeContentItem`, `number`, `url`, `file`, `string`, `com.apple.shortcuts.WFiTunesArtistContentItem`, `bool`, `date`
...
**Output Types:** `string`, `bool`, `com.apple.shortcuts.WFAppearanceSettingContentItem`
...
**Output Types:** `number`, `string`, `url`, `date`, `file`
...
**Output Types:** `number`, `url`, `com.apple.shortcuts.WFTimeIntervalContentItem`, `date`, `string`, `com.apple.shortcuts.WFEKParticipantContentItem`, `file`, `com.apple.shortcuts.WFFileSizeContentItem`, `bool`
...
**Output Types:** `file`, `com.apple.shortcuts.WFEmailAddressContentItem`, `string`, `placemark`, `url`, `com.apple.shortcuts.WFPhoneNumberContentItem`, `com.apple.shortcuts.WFFileSizeContentItem`, `date`
...
**Output Types:** `bool`, `string`, `com.apple.shortcuts.WFEmailAddressContentItem`
...
**Output Types:** `com.apple.shortcuts.WFFileSizeContentItem`, `string`, `date`
...
## Get Details of Health Sample (`is.workflow.actions.properties.health.quantity`)

Gets a specific piece of information from the health samples passed into the action.

**Output Types:** `string`, `com.apple.shortcuts.WFTimeIntervalContentItem`, `date`

| Key | Display Name | Description |
| --- | --- | --- |
| `WFContentItemPropertyName` | Detail |  |
| `WFInput` | Health Sample |  |
...
**Output Types:** `date`, `number`, `placemark`, `bool`, `com.apple.shortcuts.WFTimeIntervalContentItem`, `string`, `file`, `com.apple.shortcuts.WFFileSizeContentItem`
...
**Output Types:** `file`, `url`, `string`, `number`
...
**Output Types:** `file`, `url`, `number`, `com.apple.shortcuts.WFiTunesArtistContentItem`, `com.apple.shortcuts.WFTimeIntervalContentItem`, `bool`, `date`, `string`
...
**Output Types:** `number`, `string`, `com.apple.shortcuts.WFPhoneNumberContentItem`, `url`
...
**Output Types:** `bool`, `string`, `file`, `placemark`, `date`
...
**Output Types:** `url`, `string`, `file`, `number`
...
**Output Types:** `file`, `number`, `date`, `url`, `string`, `com.apple.shortcuts.WFTimeIntervalContentItem`
...
**Output Types:** `date`, `placemark`, `string`, `person`, `currencyAmount`
...
**Output Types:** `file`, `string`, `url`, `bool`
...
**Output Types:** `date`, `com.apple.shortcuts.WFFileSizeContentItem`, `string`, `file`, `number`
...
**Output Types:** `url`, `date`, `string`, `file`
...
**Output Types:** `com.apple.shortcuts.WFWeatherAirQualityPollutantContentItem`, `number`, `placemark`, `string`, `date`, `genericMeasurement`
...
**Output Types:** `string`, `date`, `bool`, `file`, `number`
...
**Output Types:** `com.apple.shortcuts.WFPodcastEpisodeContentItem`
...
**Output Types:** `com.apple.shortcuts.WFArticleContentItem`, `url`
...
**Output Types:** `com.apple.shortcuts.WFParkedCarContentItem`
...
**Output Types:** `com.apple.shortcuts.WFPhoneNumberContentItem`
...
**Output Types:** `com.apple.shortcuts.WFTripInfoContentItem`
...
**Output Types:** `com.apple.shortcuts.WFTrelloBoardContentItem`, `com.apple.shortcuts.WFTrelloListContentItem`, `com.apple.shortcuts.WFTrelloCardContentItem`
...
**Output Types:** `com.apple.shortcuts.WFUlyssesSheetContentItem`
...
Branch based on conditions, optionally providing Otherwise and End If blocks.

> Use `WFControlFlowMode` 0 for the opening If, 1 for Otherwise, and 2 for End If. All related blocks must share the same `GroupingIdentifier`.
> Define the condition(s) through `WFConditions` â€” each entry contains a comparator, input, and value to match.
...
| `WFQuantitySampleQuantity` | Value |  |
| `WFQuantitySampleAdditionalQuantity` | Diastolic |  |
| `WFQuantitySampleAdditionalEnumeration` | Reason |  |
| `WFCategorySampleEnumeration` | Value |  |
| `WFCategorySampleAdditionalEnumerationKey` | Start of Cycle |  |
| `WFQuantitySampleDate` | Date | The date and time of the data point. The current date will be used if you don't provide a date. |
| `WFSampleEndDate` | End Date | The date and time for the end of the data point. The current date will be used if you don't provide a date. |
...
**Output Types:** `com.apple.shortcuts.WFHKWorkoutContentItem`
...
| `WFWorkoutCaloriesQuantity` | Calories | The calories burned during the activity (optional) |
| `WFWorkoutDistanceQuantity` | Distance | The distance covered during the activity. Only provide this if it makes sense for the activity. (optional) |
...
## New Contact (`is.workflow.actions.addnewcontact`)
...
## New Event (`is.workflow.actions.addnewevent`)
...
## New Reminder (`is.workflow.actions.addnewreminder`)
...
| `WFSelectedApp` | App |  |
| `WFAppName` | App Name |  |
| `WFWindowingFormat` | Window Location & Size |  |
...
## Post on Instagram (`com.burbn.instagram.openin`)
...
Repeat the contained actions a specific number of times.

> Set `WFControlFlowMode` to 0 for the opening Repeat block and 1 for the closing End Repeat block. Both must share the same `GroupingIdentifier`.
> Use `WFRepeatCount` to control how many times the actions run. Inside the loop you can use `Repeat Index` and `Repeat Item` magic variables.

| Key | Display Name | Description |
| --- | --- | --- |
| `WFRepeatCount` | Number of Times |  |
| `GroupingIdentifier` | Grouping Identifier |  |
| `WFControlFlowMode` | Control Flow Mode |  |
| `UUID` | UUID |  |
...
Run the contained actions for every item passed into the loop.

> Opening and closing blocks share the same `GroupingIdentifier`. Set `WFControlFlowMode` to 0 for the opening Repeat with Each and 1 for the End Repeat block.
> Use the `Repeat Item` magic variable inside the loop to access the current element, or `Repeat Index` for the position.
...
**Output Types:** `bool`, `string`, `file`, `number`
...
## Search (`is.workflow.actions.spotlightsearch`)

Searches for content in the system that matches the specified text.

**Output Types:** `com.apple.shortcuts.WFContentItem`, `com.apple.shortcuts.WFCalendarEventContentItem`, `com.apple.shortcuts.WFMessageContentItem`, `person`, `file`, `com.apple.shortcuts.WFReminderContentItem`, `com.apple.shortcuts.WFEmailContentItem`, `.`

| Key | Display Name | Description |
| --- | --- | --- |
| `WFInputText` | Text |  |
| `WFSpotlightSearchLimit` | WFSpotlightSearchLimit |  |
...
## Send Photo via WhatsApp (`net.whatsapp.WhatsApp.openin`)
...
Sets AirDrop receiving to Off, Contacts Only, or Everyone for 10 Minutes. This allows you to choose who can see your device and send you content through AirDrop.
...
**Output Types:** `com.apple.shortcuts.WFShazamMediaContentItem`
...
## Show Content (`is.workflow.actions.showresult`)
...
## Show Content Graph (`is.workflow.actions.viewresult`)
...
## Use Model (`is.workflow.actions.askllm`)

Use a model to handle complex requests in your shortcuts.

**Output Types:** `com.apple.shortcuts.GeneratedContent`, `date`, `com.apple.shortcuts.WFContentItem`, `number`, `bool`, `file`

| Key | Display Name | Description |
| --- | --- | --- |
| `WFLLMPrompt` | Request | A request for the model that optionally includes variables and outputs from previous actions, including calendar events, reminders, images, and more. |
| `WFLLMModel` | Model |  |
| `FollowUp` | Follow Up | Show the model’s response and make additional requests before the final response is passed to the next action. |
| `WFGenerativeResultType` | Output | Responses are automatically optimized for the actions they’re passed into — for example, if the response is passed into the “Repeat with Each” action, the model creates a list.<br/><br/>To manually specify the model’s output, select an output type using the “Output” parameter.<br/><br/>**Private Cloud Compute Model**<br/>Use a large server-based model on Private Cloud Compute to handle complex requests while protecting your privacy.<br/><br/>**On-Device Model**<br/>Use the on-device model to handle simple requests without the need for a network connection.<br/><br/>**Extension**<br/>Tap into ChatGPT's broad world knowledge and domain expertise.<br/><br/>Image analysis is supported by the Private Cloud Compute and Extension models. |
